from django.db.models import (ExpressionWrapper, F, Q, FloatField, Sum)
...
def get_polls_by_logged_in_user(user):
    ...
    for poll in polls:
        poll_vote = PollVote.objects.filter(voter=user, poll_id=poll["id"]).values("poll_choice__choice").first()
        if poll_vote:
